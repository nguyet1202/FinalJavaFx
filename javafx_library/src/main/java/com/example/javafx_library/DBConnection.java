...
public class DBConnection {
    ...
    public DBConnection() {
        try {
            connection = DriverManager.getConnection(
                    "jdbc:mysql://localhost/library_management",
                    "root",
                    "");
    ...
    public void updateBook(Book books){
        String sql="UPDATE books SET name='"+books.name+"',image="+books.image+"',author="+books.author+"',Category="+books.Category+"',price="+books.price+"',quantityNow="+books.quantityNow+"WHERE id="+books.id;
        System.out.println(sql);
        try {
            connection.prepareStatement(sql).executeUpdate();
        } catch (SQLException e) {
